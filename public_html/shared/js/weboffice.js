 ...
 * @param height
 */
function  createWebOffice(tid,width,height){
	...
	if(navigator.userAgent.indexOf("Chrome")>0 || navigator.userAgent.indexOf("Firefox")>0){
		_s = "<object id='"+_webofid+"' type='application/x-itst-activex' align='baseline' border='0'"
			+ "style='LEFT: 0px; WIDTH: "+width+"; TOP: 0px; HEIGHT: "+height+"'"
			+ "clsid='{E77E049B-23FC-4DB8-B756-60529A35FAD5}'"
			+ "event_NotifyCtrlReady='' "
			+ "</object>";
	}else {
		_s = "<OBJECT id='"+_webofid+"' align='middle' style='LEFT: 0px; WIDTH: "+width+"; TOP: 0px; HEIGHT: "+height+"' "
...
function openFile(){
	...
	try{
		var webObj=document.getElementById(_webofid);
		webObj.OpenFileDlg();
	}catch(e){
...
function taohong(){
	var tmppath=document.all.WebOffice1.GetTempFilePath();
	document.all.WebOffice1.SaveAs(tmppath,0);
	document.all.WebOffice1.LoadOriginalFile("c:/test.doc","doc");
	document.all.WebOffice1.SetFieldValue("data", tmppath, "::FILE::");
	document.all.WebOffice1.DelLocalFile(tmppath);
}
/****************************************************
*
*					填充模板
...
*
/****************************************************/
function SaveDoc(url,params) {
	try{
		var webObj=document.getElementById(_webofid);
		webObj.HttpInit();			//初始化Http引擎
		// 添加相应的Post元素 
		$.each(params, function(key, val) {
			webObj.HttpAddPostString(key,val);
		});
		
		webObj.HttpAddPostCurrFile("FileBlod","");//上传文件
		returnValue = webObj.HttpPost(url);	// 判断上传是否成功
...
function updateAllField(){
	try{
		var webObj = document.getElementById(_webofid);
		//数据格式
		var pcFieldSettings = "STRDATA:XML_标题;;标题;6;;请输入标题...;\nXML_等级DEF;XML_等级;等级;4;普通||加急||特急;\nXML_等级;;等级;4;普通||加急||特急;\nXML_密级;;密级;4;普通||保密||机密||绝密;\nXML_电报抬头;;电报抬头;5;;请输入电报抬头...\nXML_落款单位;;落款单位;6;;请输入落款单位\nXML_落款日期;;落款日期;6;&Hou;请输入落款日期\nXML_主题词;;主题词;0;;\nPRG_CS;;抄送人;0;;\nPRG_FS;;份数;0;;\nPRG_ZW;;正文;4;这是内容结果值;请输入正文...";
		//文件路径格式
