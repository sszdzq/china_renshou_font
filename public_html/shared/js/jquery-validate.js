(function(factory) {
    ...
    var RULES = {
        ...
        onlyOne: function(value, errorMsg, el) {//检验用户名是否重复
        var msg="";
        	var url=$(el).attr("checkUrl");
        	//console.log(url);
        	var flag=false;
        ...
        onlyRoleName: function(value, errorMsg, el) {//检验用户名是否重复
            var msg="";
            	var url=$(el).attr("checkUrl");
            	//console.log(url);
            	var flag=false;
        		...
        		$.ajax({
        				type : "get",
        				url :url+"?name="+value+"&noCache="+new Date().getTime(),
        				data : {},
        				contentType:"application/json",
        				dataType : "json",
        				async:false,
        ...
        isPfxOrP12: function(value, errorMsg, el) { //判断服务器证书是否格式zhengque
          if(value.lastIndexOf(".pfx")!=-1){
        	  
          }else{
        	  return errorMsg;
          }
        },
        isCer: function(value, errorMsg, el) { //判断服务器证书是否格式zhengque
            if(value.lastIndexOf(".cer")!=-1){
          	  
            }else{
          	  return errorMsg;
            }
          },
          isUlEmpty: function(value, errorMsg,el) {
          	var obj=$(el).find('li');
