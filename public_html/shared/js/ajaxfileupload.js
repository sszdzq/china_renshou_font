jQuery.extend({
    ...
    ajaxFileUpload: function(s) {
        ...
        //创建锁定页面的div层
        if(utils.isNull(s.lockPage)==false&&s.lockPage===true){
			...
			window.parent.jQuery("body").append(val);
		}
        
        var id = new Date().getTime();
        var form = jQuery.createUploadForm(id, s.fileElementId);
        //增加页面验证参数
        if(s.data==null||typeof s.data ==="undefined"){
        	s.data={_jspage:utils.getUrlPath()};
        }else{
        	s.data["_jspage"]=utils.getUrlPath();
        }
    ...
    uploadHttpData: function( r, type ) {
        ...
        if ( type == "json" ){
            ...
            var data = (am!=null) ? am[1] : data;    //the only submatch or empty
            try{
